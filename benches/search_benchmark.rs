...
fn benchmark_indexing(c: &mut Criterion) {
    ...
    c.bench_function("index_1000_files", |b| {
        b.iter(|| {
            ...
            black_box(indexer.build_index(temp_dir.path().to_str().unwrap()).unwrap())
        })
    });
}
...
fn benchmark_substring_search(c: &mut Criterion) {
    ...
    c.bench_function("substring_search", |b| {
        b.iter(|| {
            black_box(search_engine.search_substring(&index, "test"))
        })
    });
}
...
fn benchmark_regex_search(c: &mut Criterion) {
    ...
    c.bench_function("regex_search", |b| {
        b.iter(|| {
            black_box(search_engine.search_regex(&index, r"test_file_\d+\.txt").unwrap())
        })
    });
}

criterion_group!(benches, benchmark_indexing, benchmark_substring_search, benchmark_regex_search);
